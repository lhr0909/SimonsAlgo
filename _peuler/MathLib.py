...
#prime factorization (easiest one)
...
def MillerRabin(n):
    ...
    if n < 1373653:
        ...
        A = [2, 3, 5, 7, 11, 13, 17]
    #if you want a large number, use [2, 3, 5, 7, 11, 13, 17]
    for a in A:
        flag = False
        x = pow(a, d, n)
        if x==1 or x==n-1:
...
def perm_iter(elements, length):
    for term in choose_iter(elements, length):
        yield term
        number = list(term[:])
        n = length
        for a in range(product(range(1, length+1)) - 1):
            i = n - 1
